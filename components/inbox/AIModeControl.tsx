...
export default function AIModeControl({ conversation, onModeChange }: Props) {
  ...
  const setMode = async (newMode: string) => {
    ...
    setLoading(true);
    try {
      await api.functions.invoke('toggleAIMode', { conversation_id: conversation.id, mode: newMode });
  ...
  return (
    <div className="flex items-center gap-1 rounded-full bg-[#e9edef] p-0.5">
      {buttons.map(({ key, label, Icon, activeClass, inactiveClass }) => (
        <button key={key} disabled={loading} onClick={() => setMode(key)}
          className={cn('flex items-center gap-1 px-2.5 py-1.5 rounded-full text-xs font-semibold transition-all', mode === key ? activeClass : inactiveClass, loading && 'opacity-50 cursor-not-allowed')}
          style={{ minHeight: 32 }}>
