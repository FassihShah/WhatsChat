...
import { Bot, StickyNote, CheckCheck, Check, AlertCircle, Download, Play } from 'lucide-react';
...
function Skeleton({ width, height }: { width: number; height: number }) {
  return <div style={{ width, height, borderRadius: 12, background: '#e0e0e0', animation: 'pulse 1.5s ease-in-out infinite' }} />;
}
...
function MediaBubble({ msg, isSent, userId }: { msg: any; isSent: boolean; userId?: string }) {
    ...
    <div ref={wrapperRef}>
        ...
        <>
          ...
          {dataUrl && (
            <div style={{ position: 'relative', display: 'inline-block' }}>
              <img src={dataUrl} alt="image" onClick={() => setLightbox(true)} style={{ borderRadius: 12, display: 'block', cursor: 'pointer', maxWidth: 'min(260px, 85vw)', maxHeight: 300, width: '100%', objectFit: 'cover' }} />
              <button onClick={() => downloadFile(dataUrl, filename)} style={{ position: 'absolute', bottom: 8, right: 8, width: 32, height: 32, borderRadius: '50%', background: 'rgba(0,0,0,0.5)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                <Download style={{ width: 14, height: 14, color: 'white' }} />
              </button>
            </div>
          )}
          {lightbox && dataUrl && <MediaLightbox url={dataUrl} type="image" filename={filename} onClose={() => setLightbox(false)} />}
        </>
      )}
      ...
      {mediaType === 'video' && (
        <>
          ...
          {dataUrl && (
            <div style={{ position: 'relative', display: 'inline-block', cursor: 'pointer' }} onClick={() => setLightbox(true)}>
              <video src={dataUrl} preload="metadata" muted style={{ borderRadius: 12, maxWidth: 'min(260px, 85vw)', maxHeight: 200, display: 'block' }} />
          ...
          {lightbox && dataUrl && <MediaLightbox url={dataUrl} type="video" filename={filename} onClose={() => setLightbox(false)} />}
        </>
      )}
      ...
      {mediaType === 'document' && (
        <>
          ...
          {dataUrl && (() => {
            const { bg, color, label } = getDocStyle(filename);
            return (
              <div style={{ borderRadius: 12, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 12, maxWidth: 'min(260px, 85vw)', cursor: 'pointer', border: '1px solid rgba(0,0,0,0.1)', background: isSent ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.03)' }} onClick={() => downloadFile(dataUrl, filename)}>
                ...
                <div style={{ flex: 1, overflow: 'hidden' }}>
                  <div style={{ fontWeight: 500, fontSize: 13, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{filename}</div>
                  <div style={{ fontSize: 11, color: '#667781' }}>{label} file</div>
                </div>
                <button onClick={(e) => { e.stopPropagation(); downloadFile(dataUrl, filename); }} style={{ width: 32, height: 32, background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
export default function MessageBubble({ msg, userId, conversation }: { msg: any; userId?: string; conversation?: any }) {
  ...
  if (isSystem && !isNote) {
    return (
      <div className="flex justify-center my-2 px-4">
        <p style={{ fontSize: 12, color: '#adb5bd', fontStyle: 'italic', textAlign: 'center' }}>{msg.content}</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex justify-center my-3">
        <div className="bg-[#fff9c4] border border-[#f0e68c] rounded-lg px-4 py-2.5 max-w-sm shadow-sm">
          <div className="flex items-center gap-1.5 mb-1"><StickyNote className="w-3 h-3 text-[#b8960c]" /><span className="text-[11px] font-semibold text-[#b8960c]">Internal Note</span>{msg.agent_name && <span className="text-[11px] text-[#9a7d0a]">· {msg.agent_name}</span>}</div>
    ...
    <div className={cn('flex mb-1', isSent ? 'justify-end' : 'justify-start')}>
      {isCustomer && (
        <div className="w-7 h-7 rounded-full bg-[#dfe5e7] flex items-center justify-center shrink-0 mr-1.5 mt-auto mb-0.5">
          <span className="text-[11px] font-bold text-[#54656f]">{(conversation?.customer_name || conversation?.customer_phone || 'C')[0].toUpperCase()}</span>
        </div>
      )}
      <div style={{ maxWidth: 'min(65%, 85vw)' }}>
        {isSent && msg.sender === 'ai' && (
          <div className="flex items-center gap-1 mb-0.5 justify-end px-1"><Bot className="w-3 h-3 text-blue-400" /><span className="text-[10px] text-blue-500 font-medium">AI Agent</span></div>
        )}
        <div className={cn('relative px-3 py-2 shadow-sm', isSent ? 'bg-[#dcf8c6] text-[#111b21] rounded-tl-2xl rounded-bl-2xl rounded-br-2xl' : 'bg-white text-[#111b21] rounded-tr-2xl rounded-br-2xl rounded-bl-2xl')}>
          {isSent ? <div className="absolute -right-1.5 top-0 w-0 h-0 border-l-[8px] border-l-[#dcf8c6] border-t-[8px] border-t-transparent" /> : <div className="absolute -left-1.5 top-0 w-0 h-0 border-r-[8px] border-r-white border-t-[8px] border-t-transparent" />}
          {isMedia ? (
            <div>
              ...
              <div className="flex items-center justify-end gap-0.5 mt-1"><span className="text-[11px] text-[#111b21]/60">{timeStr}</span>{renderStatus()}</div>
            </div>
          ) : (
            <div>
